...
{
	{
		QSqlDatabase db = QSqlDatabase::database(this->connectName);
		QSqlQuery query(db);
		query.prepare("SELECT userAccount FROM usersinfo");
		if (query.exec()) {
			while (query.next()) {
				QString account = query.value(0).toString();
				if (account == userAccount) {
					query.prepare("SELECT userName,userHead,userPassword FROM usersinfo WHERE userAccount = " + userAccount);
					if (query.exec()) {
						while (query.next()) {
...
{
	{
		QSqlDatabase db = QSqlDatabase::database(this->connectName);
		QSqlQuery query(db);
		query.prepare("SELECT usersinfo.userAccount FROM usersinfo;");
		if (query.exec()) {
			while (query.next()) {
...
void UserDatabaseManager::RegisterUserAccount(const QByteArray& imageBytes, const QString& userName, const QString& userPassword)
{
	...
	while (true) {
		...
		bool isExist = this->isExistTheSameUserAccount(account);
		if (!isExist)
			break;
	}
	QSqlDatabase db = QSqlDatabase::database(this->connectName);
	QSqlQuery query(db);
	query.prepare("INSERT INTO usersinfo (userName,userHead,userAccount,userPassword,onlineStatus) VALUES(:username,:userimagebytes,:useraccount,:userpassword,:status)");
...
void  UserDatabaseManager::selectUserDataForSearch(const QString& userAccount)
{
	...
	SearchFriendData data;
	{
		QSqlDatabase db = QSqlDatabase::database(this->connectName);
		QSqlQuery query(db);
		query.prepare("SELECT userName,userHead,onlineStatus FROM usersinfo WHERE userAccount = " + userAccount);
		if (query.exec()) {
			while (query.next()) {
				QString username = query.value(0).toString();
				QByteArray imagebytes = query.value(1).toByteArray();
...
bool UserDatabaseManager::isExistTheSameUserApplication(const QString& receiver) const
{
	...
	QSqlDatabase db = QSqlDatabase::database(this->connectName);
	QSqlQuery query(db);
	query.prepare("SELECT receiver From friendapplication WHERE sender = " + QString::number(GLOB_UserAccount));
	if (query.exec()) {
		while (query.next()) {
...
void UserDatabaseManager::increaseUserApplicationTemp(const QString& receiver)
{
	...
	if (!ret) {
		QSqlDatabase db = QSqlDatabase::database(this->connectName);
		QSqlQuery query(db);
		query.prepare("INSERT INTO friendapplication (sender,receiver) VALUES (:sender,:receiver);");
		...
		if (query.exec()) {
			...
			query.prepare("SELECT onlineStatus FROM usersinfo WHERE userAccount = " + receiver);
			if (query.exec()) {
				while (query.next()) {
...
bool UserDatabaseManager::isExistTheUser(const QString& userAccount) const
{
	QSqlDatabase db = QSqlDatabase::database(this->connectName);
	QSqlQuery query(db);
	query.prepare("SELECT friendAccount FROM userfriends WHERE userAccount = " + QString::number(GLOB_UserAccount));
	if (query.exec()) {
		while (query.next()) {
			QString account = query.value(0).toString();
			if (account == userAccount)
...
UserData UserDatabaseManager::GetUserFriendData(const QString& cronyAccount) const
{
	...
	QSqlDatabase db = QSqlDatabase::database(this->connectName);
	QSqlQuery query(db);
	query.prepare("SELECT userName,userHead,onlineStatus FROM usersinfo WHERE userAccount = " + cronyAccount);
	if (query.exec()) {
		while (query.next()) {
			QString username = query.value(0).toString();
			QByteArray imagebytes = query.value(1).toByteArray();
...
void UserDatabaseManager::addUserFriend(const QString& userAccount)
{
	...
	if (ret && query.exec()) {
		...
		query.prepare("DELETE FROM friendapplication WHERE sender = " + userAccount + " AND receiver = " + QString::number(GLOB_UserAccount));
		if (query.exec()) {
...
void UserDatabaseManager::selectUserHeadData(const QString& userAccount)
{
		...
		return;
	{
		QSqlDatabase db = QSqlDatabase::database(this->connectName);
		QSqlQuery query(db);
		query.prepare("SELECT usersinfo.userHead FROM usersinfo WHERE usersinfo.userAccount =" + userAccount);
		if (query.exec()) {
			while (query.next()) {
