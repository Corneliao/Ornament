...
CustomGraphicsView::CustomGraphicsView(QWidget* parent)
{
	...
	this->scaleAnimation = new QPropertyAnimation(this, "zoom", this);
	this->scaleAnimation->setDuration(700);
	this->scaleAnimation->setEasingCurve(QEasingCurve::InOutQuad);
	connect(this->scaleAnimation, &QPropertyAnimation::valueChanged, this, [=](QVariant value) {
		double fator = value.toDouble() / this->m_zoom;
		if (fator == this->m_zoom) {
			return;
		}

		this->m_zoom = value.toDouble();
		this->scale(fator, fator);
		});
}

double CustomGraphicsView::zoom()const {
	return this->m_zoom;
}
void CustomGraphicsView::setZoom(double newZoom)
{
	this->scale(newZoom, newZoom);
}
...
void CustomGraphicsView::wheelEvent(QWheelEvent* event)
{
	...
	//this->scaleAnimation->setStartValue(this->m_zoom);
	//this->scaleAnimation->setEndValue(1.1);
